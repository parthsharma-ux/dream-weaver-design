...
CREATE POLICY "Admins can update services" ON public.services FOR UPDATE TO authenticated USING (has_role(auth.uid(), 'admin'::app_role));
...
CREATE POLICY "Admins can insert portfolio" ON public.portfolio_projects FOR INSERT TO authenticated WITH CHECK (has_role(auth.uid(), 'admin'::app_role));
CREATE POLICY "Admins can update portfolio" ON public.portfolio_projects FOR UPDATE TO authenticated USING (has_role(auth.uid(), 'admin'::app_role));
CREATE POLICY "Admins can delete portfolio" ON public.portfolio_projects FOR DELETE TO authenticated USING (has_role(auth.uid(), 'admin'::app_role));
